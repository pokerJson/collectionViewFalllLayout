...
class DGNewRectingVC: UIViewController{
    
    private var newRectingList = [DGHotRectingModel]()//热门跟读数组
...
//    }
    ...
    private func getHotRectingData(){
        ...
        DGServer.getNewRectingData(pageN: pageNum,materialId: materialIdStr).onSuccess { [weak self] newRectinglistModel in
            guard let self = self else { return }
            self.collectionView.mj_footer.endRefreshing()

            guard self.pageNum != 1 else {
                //第一页
                self.newRectingList.removeAll()
                if let list = newRectinglistModel.list,list.isEmpty == false {
                    //有数据
                    self.noDataView.isHidden = true
                    self.newRectingList.append(contentsOf: list)
                    self.collectionView.reloadData()
                    if self.pageNum == newRectinglistModel.totalPage {
...
extension DGNewRectingVC: WaterfallMutiSectionDelegate{
    func heightForRowAtIndexPath(collectionView collection: UICollectionView, layout: DGWaterfallFlowLayout, indexPath: IndexPath, itemWidth: CGFloat) -> CGFloat {
        return newRectingList[indexPath.row].cellHeight
    }
...
extension DGNewRectingVC: UICollectionViewDelegate,UICollectionViewDataSource{
    func numberOfSections(in collectionView: UICollectionView) -> Int {
      return 1
    }

    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return newRectingList.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.fillData(model: newRectingList[indexPath.row])
        return cell
    }
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        print("点击的是-->\(indexPath.row)")
        let vc = DGRectingDetailVC(nodeId: newRectingList[indexPath.row].iid,isFromHotOrNewRecting: true)
